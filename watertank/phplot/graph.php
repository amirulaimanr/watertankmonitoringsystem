...
$dt_graph=$_GET["date"];
$sql="SELECT `date_time`, `sensor2` FROM `logs1` where `date_time` like '%$dt_graph%' ORDER BY `date_time`";
$result = create_db_connection()->query($sql);
...
for($i=0;$i<1440;$i++)
  {
	...
	if($val==""){ //if there is no value found for this key (minute), it means data not recorded for this minute
		...
		global $dia, $height, $level;
		$h=$height-$val; 
		$vol=calculate_volume($dia,$h);
		$ass_array_padded[$min]=$val;
	}
    //echo"$min = $val<br>";
  } 
...
function draw_graph($arr2d){
    $plot = new PHPlot(550, 390); //size of graph 
    ...
    $title='Water Tank Level (cm/hrs) : '.$dt;
    $plot->SetTitle($title);
